...
void dns_server(void *pvParameters)
{
    ...
    LOGI(TAG, "DNS Server listening on 53/udp");
    ...
    for(;;) {
        ...
        if ( length > 0   &&  ((length + sizeof(dns_answer_t)-1) < DNS_ANSWER_MAX_SIZE)   )
        {
            ...
            LOGI(TAG, "Replying to DNS request for %s from %s", domain, ip_address);
            ...
            if (err < 0) {
            	LOGE(TAG, "UDP sendto failed: %d", err);
            }
        }
