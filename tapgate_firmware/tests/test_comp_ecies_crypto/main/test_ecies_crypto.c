//
// Unit tests for ECIES encryption module
//
...
#include "../../components/ecies_crypto/ecies.h"

// Host key pair - used to decrypt client-generated messages
...
static const uint8_t host_public_key[] __attribute__((unused)) = {
...
static const uint8_t client_public_key[] __attribute__((unused)) = {
...
// Test: Encrypt and decrypt a natively generated message
TEST_CASE("test ecies encrypt/decrypt self-generated message", "[ecies]")
{
    uint8_t gen_private_key[ECIES_X25519_KEY_SIZE];
    uint8_t gen_public_key[ECIES_X25519_KEY_SIZE];

    bool result = ecies_generate_keypair(gen_private_key, gen_public_key);
    ...
    result = ecies_encrypt((const uint8_t *)plaintext, plaintext_len,
                           gen_public_key, ciphertext, sizeof(ciphertext), &ciphertext_len);
    TEST_ASSERT_TRUE(result);
    ...
    result = ecies_decrypt(ciphertext, ciphertext_len, gen_private_key,
    ...
    printf("Test 1 passed: self-generated message encrypted and decrypted successfully\n");
    printf("  Original:  \"%s\"\n", plaintext);
    printf("  Decrypted: \"%s\"\n", (const char *)decrypted);
}

// Test: Decrypt message encoded on client (MAUI) with host_public_key
// plaintext: "Test message encoded on MAUI client"
TEST_CASE("test ecies encrypt/decrypt client-generated message", "[ecies]")
{
    ...
    static const char expected_plaintext[] = "Test message encoded on MAUI client";

    uint8_t decrypted[128];
    size_t  decrypted_len = 0;
    size_t  expected_len  = strlen(expected_plaintext);

    bool result = ecies_decrypt(ciphertext, sizeof(ciphertext), host_private_key,
                                decrypted, sizeof(decrypted), &decrypted_len);
    TEST_ASSERT_TRUE(result);
    TEST_ASSERT_EQUAL_INT(expected_len, decrypted_len);
    TEST_ASSERT(memcmp(expected_plaintext, decrypted, expected_len) == 0);

    printf("Test 2 passed: client-generated message decrypted successfully\n");
